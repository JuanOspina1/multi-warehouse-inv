...
import { OverviewActiveWarehouses } from "src/sections/overview/overview-active-warehouses";
...
const Page = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe1 = onSnapshot(qOrders, (querySnapshot) => {
      const orders = [];
    ...
    const unsubscribe2 = onSnapshot(qWarehouses, (querySnapshot) => {
      const warehouses = [];

      querySnapshot.forEach((doc) => {
        warehouses.push({
          name: doc.data().information[0].name,
          phone: doc.data().information[0].phone,
          id: doc.id,
        });
      });
